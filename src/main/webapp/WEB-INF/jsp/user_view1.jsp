		...
		<div class="content-body" style="min-height: 884px;">
			...
			<div class="container-fluid">
				<div class="d-flex align-items-center mb-4 flex-wrap">
					...
					<c:if test="${COLLEGEUSER_SESSION.role==1 }">
					<a href="${pageContext.request.contextPath }/findUserById_m?id=${user.id}" class="btn btn-primary me-3 btn-sm">
						编辑
					</a>
					</c:if>
								...
								<div class="mb-md-2 mb-3">
									<span class="d-block mb-1">
										<i class="fas fa-circle me-2"></i>
											<c:if test="${user.resumeStatus==0 }">
												<span class="font-w400">该用户还未投递过简历</span>
											</c:if>
											<c:if test="${user.resumeStatus==1 }">
												<span class="font-w400">该用户已投递过简历</span>
											</c:if>
									</span>
								</div>
								<div class="card-footer text-end">
								<div>
									<a href="${pageContext.request.contextPath }/mailToSendResume?id=${user.id}">
									<button class="btn btn-primary me-3 btn-sm" value="邀请发送简历">邀请发送简历</button>
									</a>
								</div>
							</div>
							</div>
